	...
	if response.candidates[0].finishReason != "STOP":
		...
		for item in  response.candidates[0].content.parts:
			if item.has("executableCode"):
				var language = item.executableCode.language
				newStr+="<Start executableCode language='%s'>\n"%language
				newStr+=item.executableCode.code
				newStr+="</End executableCode>\n"
			elif item.has("codeExecutionResult"):
				var outcome = item.codeExecutionResult.outcome
				newStr+="<Start codeExecutionResult outcome='%s'>\n"%outcome
				newStr+=item.codeExecutionResult.output
				newStr+="</End codeExecutionResult>\n"
			elif item.has("text"):
				var text = item.text.strip_edges()
				if text!="": 
					newStr+="<Start Text>\n"
					newStr+=item.text
					newStr+="</End Text>\n"
